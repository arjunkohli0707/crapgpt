...
"""
TASKS:
- converting .biom.qza into a .biom file so that you can use load table?
- extract the asvs from the now biom table using the get_sequencing_dataset function
below 
- generalize the function below such that it doesn't use tensorflow, but it uses numpy. WHY?
- 
"""

def get_sequencing_dataset(table_path):
    if type(table_path) == str:
        table = load_table(table_path) # after converting .biom.qza into a .biom file
    else:
        table = table_path
    o_ids = tf.constant(table.ids(axis='observation')) # grabs the column of asvs
    print(o_ids)
